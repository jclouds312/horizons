    ...
    <PageWrapper>
      ...
      <div className="min-h-screen bg-white py-8">
        <div className="max-w-7xl mx-auto px-4">
          <h1 className="text-3xl font-bold text-primary mb-8 text-center">Nuestros Vendedores</h1>
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-6">
            {sellers.map((seller) => (
              <div
                key={seller.id}
                onClick={() => navigate(`/seller/${seller.id}`)}
                className="bg-white border border-gray-200 rounded-lg p-4 flex flex-col items-center text-center hover:shadow-md hover:-translate-y-1 transition-all duration-300 cursor-pointer"
              >
                <img
                  src={seller.avatar}
                ...
                />
                <h2 className="text-lg font-semibold text-primary">{seller.name}</h2>
